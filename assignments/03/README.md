...
This challenge code uses "safe" string-copy function strncpy(), so you won't be able to use the same approach as for victim2. Instead, you'll have to find a more devious way to get the shellcode to execute. As with victim2, your goal is to obtain a root shell, using the provided shellcode.

### Challenge 4 [15 bonus points]: File-based shellcode
...
Challenge #8 is a network service that reads messages from a socket and sends a message back. Assuming that you do not have access to the machine where this programs run, use your hacking skills to install a backdoor on that remote server.
